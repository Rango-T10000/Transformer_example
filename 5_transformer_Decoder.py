...
class Pos_FFN(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.lin1 = nn.Linear(24,48, bias = False)
        self.relu1 = nn.ReLU()
        self.lin2 = nn.Linear(48,24, bias = False)
        self.relu2 = nn.ReLU()       
...
class Decoder_block(nn.Module):
    ...
    def forward(self, X, X_en):
        X_1 = self.attention(X)
        X = self.add_norm_1(X, X_1)
        X_2 = self.cross_attention(X, X_en)
        X = self.add_norm_2(X, X_2)
        X_3 = self.FFN(X)
        X = self.add_norm_3(X, X_3)
        return X
...
class Decoder(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.decoder_blks.append(Decoder_block())
        self.dense = nn.Linear(24, 28, bias = False)  #过完所有的Decoder_blk，最终还要经过一个MLP


    def forward(self, X, X_en):
        X = self.ebd(X)
        for decoder_blk in self.decoder_blks:
            X = decoder_blk(X, X_en)
        return X